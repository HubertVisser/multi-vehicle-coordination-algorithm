    ...
    def call(self, state, model, params, is_feasible):
        ...
        for k in [0]:  # [0, self._N]:
            for i in range(self._max_constraints):
                params.set(k, f"disc_0_decomp_{i}_a1", 1.0)
                params.set(k, f"disc_0_decomp_{i}_a2", 0.0)
                params.set(k, f"disc_0_decomp_{i}_b", state[0] + 100.0)
        ...
        response = self.get_decomp_constraints(
            request.x, request.y, request.first, request.last
        )
        ...
        for k in range(1, self._N + 1):
            ...
            for i in range(num_constraints):
                params.set(
                    k, f"disc_0_decomp_{i}_a1", response.constraint_a1[start + i]
                )
                params.set(
                    k, f"disc_0_decomp_{i}_a2", response.constraint_a2[start + i]
                )
