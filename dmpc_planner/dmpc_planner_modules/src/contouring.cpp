...
namespace MPCPlanner
{
  ...
  void Contouring::setParameters(const RealTimeData &data, const ModuleData &module_data, int k)
  {
    ...
    static double terminal_angle_weight, terminal_contour_weight, terminal_lag_weight;
    ...
    if (k == 0)
    {
      ...
      terminal_contour_weight = CONFIG["weights"]["terminal_contour"].as<double>();
      terminal_lag_weight = CONFIG["weights"]["terminal_lag"].as<double>();
...

    {
      ...
      setSolverParameterTerminalContour(k, _solver->_params, terminal_contour_weight);
      setSolverParameterTerminalLag(k, _solver->_params, terminal_lag_weight);
